 ...
 */
public class GadirStrutsParamsInterceptor extends MethodFilterInterceptor {

    private static final Logger LOG = LoggerFactory.getLogger(GadirStrutsParamsInterceptor.class);
    ...
    protected void setParameters(Object action, ValueStack stack, final Map<String, Object> parameters) {
        ...
        for (Map.Entry<String, Object> entry : acceptableParameters.entrySet()) {
            ...
            //jbenitac
//            try{
//            	if (value instanceof String)value = new String(((String)value).getBytes("ISO-8859-1"), "UTF-8");
//            	if (value instanceof String[]){
//            		for (int i=0; i < ((String[])value).length ; i++){
//            			((String[])value)[i] = new String(((String[])value)[i].getBytes("ISO-8859-1"), "UTF-8");
//            		}
//            	}
//            	
//            }catch(Exception e){
...
//            }
            ...
            try {
                ...
                if (devMode) {
                    String developerNotification = LocalizedTextUtil.findText(GadirStrutsParamsInterceptor.class, "devmode.notification", ActionContext.getContext().getLocale(), "Developer Notification:\n{0}", new Object[]{
...
public class GadirStrutsParamsInterceptor extends MethodFilterInterceptor
{
	...
	private ValueStackFactory valueStackFactory;
	static final Comparator<String> rbCollator = new Comparator() {
		public int compare(String s1, String s2) {
			int l1 = 0; int l2 = 0;
			for (int i = s1.length() - 1; i >= 0; --i) {
				if (s1.charAt(i) != '.') continue; ++l1;
			}
			for (i = s2.length() - 1; i >= 0; --i) {
				if (s2.charAt(i) != '.') continue; ++l2;
			}
			return ((l2 < l1) ? 1 : (l1 < l2) ? -1 : s1.compareTo(s2));
		}
	};

	public GadirStrutsParamsInterceptor()
	{
		this.ordered = false;
		this.excludeParams = Collections.emptySet();
		this.acceptParams = Collections.emptySet();

		this.acceptedParamNames = "[[\\p{Graph}\\s]&&[^,#:=]]*";
	...
	throws Exception
	{
		...
		if (!(action instanceof NoParameters)) {
			...
			if (LOG.isDebugEnabled()) {
				LOG.debug("Setting params " + getParameterLogMap(parameters), new String[0]);
			}
