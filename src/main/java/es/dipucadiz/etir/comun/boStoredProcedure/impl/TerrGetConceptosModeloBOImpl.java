...
public class TerrGetConceptosModeloBOImpl extends StoredProcedure implements TerrGetConceptosModeloBO {

	private static final Log LOG = LogFactory.getLog(TerrGetConceptosModeloBOImpl.class);

	public TerrGetConceptosModeloBOImpl(final DataSource dataSource) {
		super(dataSource, "CONTROL_TERRITORIAL.getConceptosPorModelos");
		setFunction(true);
		...
		declareParameter(new SqlParameter("e_codigoModelo", Types.VARCHAR));
		//declareParameter(new SqlParameter("e_tipo", Types.CHAR));
		compile();
	}
	
	@SuppressWarnings("unchecked")
	public List<Map<String, Object>> execute(final String provincia, final String municipio, final String codigoTerritorial, final String codigoModelo) {
		...
		params.put("e_codigoModelo", codigoModelo);
		//params.put("e_tipo", "G");
		List<Map<String, Object>> results = null;
		try {
			results = (List<Map<String, Object>>) execute(params).get("result");
		} catch (Exception e) {
			LOG.debug("Fallo en TerrGetConceptosModeloBOImpl.execute()", e);
		}
		return results;
	}
