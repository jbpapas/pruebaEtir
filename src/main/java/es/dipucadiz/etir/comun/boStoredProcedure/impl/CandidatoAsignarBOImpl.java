...
public class CandidatoAsignarBOImpl extends StoredProcedure implements CandidatoAsignarBO {

	private static final Log LOG = LogFactory.getLog(CandidatoAsignarBOImpl.class);

	public CandidatoAsignarBOImpl(final DataSource dataSource) {
		super(dataSource, "G533_OBTENCION_DE_CANDIDATOS.ASIGNAR_CANDIDATO");
		setFunction(true);
	...
	public Map<String, Object> execute(String coModelo, String coVersion, String coDocumento, Long coCliente, String coProcesoActual) {
		...
		params.put("e_co_cliente", coCliente.toString());
		Map<String, Object> resultsList= new HashMap<String, Object>();
		try {
			resultsList = execute(params);
		} catch (Exception e) {
			LOG.debug("Fallo en CandidatoAsignarBOImpl.execute()", e);
		}
