...
import es.dipucadiz.etir.comun.boStoredProcedure.DeudaFechaBO;
...
public class DeudaFechaBOImpl extends StoredProcedure implements DeudaFechaBO {

	private static final Log LOG = LogFactory.getLog(DeudaFechaBOImpl.class);

	public DeudaFechaBOImpl(final DataSource dataSource) {
		super(dataSource, "EJ_CALCULO.DEUDA_A_UNA_FECHA");
		LOG.info("EJ_CALCULO.DEUDA_A_UNA_FECHA");
		setFunction(true);
	...
	public DeudaVO execute(String coModelo, String coVersion, String coDocumento, Date fecha) throws GadirServiceException {
		...
		try {
			...
			if (resultsList == null) {
				throw new GadirServiceException("EJ_CALCULO.DEUDA_A_UNA_FECHA devuelve null.");
			} else {
				if (!resultsList.containsKey("resultado")) {
					throw new GadirServiceException("EJ_CALCULO.DEUDA_A_UNA_FECHA no contiene resultado.");
				} else {
