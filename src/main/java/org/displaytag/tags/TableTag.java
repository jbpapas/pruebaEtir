 ...
 * @version $Revision: 1.4 $ ($Author: jbenitac $)
 * @version 02/12/2009 FJTORRES Se modifica la exclusión de parametros para que acepte el wildcard * en palabras, 
 * de este modo excluir parametros que empiecen o terminen con la palabra que acompañe a *.
 */
...
public class TableTag extends HtmlTableTag
{
            ...
            else
            {
                for (int j = 0; j < splittedExcludedParams.length; j++)
                {
					...
					if (sep.endsWith("*")) {
						...
						Iterator paramsIterator = baseHref.getParameterMap()
								.keySet().iterator();
						while (paramsIterator.hasNext()) {
							String key = (String) paramsIterator.next();
    ...
    {
        String filename = ActionContext.getContext().getName() + properties.getExportFileName(this.currentMediaType);
