...
/*
 * Para depurar, es recomendado pasar este fichero primero por http://jsbeautifier.org/
 */
...
case k.KEY_DOWN_ARROW:
	...
	if (this.extraArgs["isusuarioaccesible"] == 'true') {
		opcionesComboAccesible(this); // Si usuario accesible, mostrar combo accesible.
	} else {
		this._highlightNextOption();  // En caso contrario, seleccionar siguiente elemento.
	}
	dojo.event.browser.stopEvent(evt);
	return;
case k.KEY_UP_ARROW:
...
//jbenitac
else if(this.popupWidget.isShowingNow && this.textInputNode.value!='' && this.optionsListNode.childNodes.length==1){
	this._selectOption({"target":this.optionsListNode.childNodes[0],"noHide":false});
	this._hideResultList();
}
//fin jbenitac
else{
...
if(this.popupWidget.isShowingNow){
dojo.event.browser.stopEvent(evt);
//jbenitac si se aprieta intro y solo hay una opcion en la lista, la selecciono
if(this.textInputNode.value!=''){
	if (this.optionsListNode.childNodes.length==1){
		this._selectOption({"target":this.optionsListNode.childNodes[0],"noHide":false});
		//this._selectOption();
		this._hideResultList();
	}
...
this._handleKeyEvents(evt);
	...
	if ((evt.keyCode == evt.KEY_BACKSPACE || evt.keyCode == evt.KEY_DELETE) && this.forceValidOption && !this._isValidOption()) {
...
this.setValue(_c4b);
},_focusOptionNode:function(node){//focus node!!
...
if (this.extraArgs["isoptionscontitle"] == 'true'){
	td.setAttribute("title", tr[0]);
}
...
this._handleBlurTimer(true,0);
//jbenitac: cambio el orden de tryfocus
...
this.setData(data);
//jronnols, autoselect si sólo hay 1 elemento en la lista buscada. ini
if (
		this.cbox.extraArgs.isrequired == 'true' &&
		dojo.lang.isArray(data[0]) &&
		data.length > 0 &&
		(
			(data.length == 1 && //En FF
			data[0][0] != '' && 
			...
			data[0][0] != '' && 
			data[0][1] != '' && 
			typeof(data[1][0]) == 'undefined' && 
			typeof(data[1][1]) == 'undefined')
		)
) {
	this.cbox.selectedResult = data[0];
	this.cbox.setAllValues(data[0][0], data[0][1]);
	//this.cbox._tryFocus();
	this.cbox.jbenitac_focus();
}
//jronnols fin
}),mimetype:"text/json"});
};
...
break;
case "load":
	//alert("Data notificada: " + data);//jronnols
	//alert(data.length);
this.notifyTo(this.afterNotifyTopicsArray,[data,e,this]);
	//alert("childNodes: " + this.optionsListNode.childNodes.length);
break;
case "error":
...
this.onSetTime();
	...
	if(evt&&evt.target){
		...
		if(!isNaN(evt)){
			var min_pos = parseInt(evt/5);
			if (min_pos*5 == evt) {
				min_pos = this.minuteIndexMap[min_pos];
				var _dd7=this.minuteContainerNode.getElementsByTagName("td");
				if(_dd7.item(min_pos)){
					dojo.html.setClass(_dd7.item(min_pos),this.classNames.selectedTime);
					this.selectedTime["minute"]=_dd7.item(min_pos).innerHTML;
				}
			} else {
				this.selectedTime["minute"]=evt;
			}
		}
	}
...
this.setDate(_e12);
	...
	if(_e13=="" || _e13==null){ /* jronnols: Añadido null para controlar 01/01/1970 */
		this.value="";
...
this._syncValueNode();
	...
	/* jronnols: Control de error 01/01/1970  --v*/
	var tmp="";
	if (this.datePicker.value.getFullYear && this.datePicker.value.getFullYear() > this.datePicker.startDate) {
		tmp = this.datePicker.value?dojo.date.format(this.datePicker.value,{formatLength:this.formatLength,datePattern:this.displayFormat,selector:"dateOnly",locale:this.lang}):"";
	}
	this.inputNode.value=tmp;
	/* jronnols: Control de error 01/01/1970  --^*/
...
this.inputNode.tabIndex=this.tabIndex;
	...
	var _e65="";
	/* jronnols: Control de error 01/01/1970  --v*/
	if (date.getFullYear && date.getFullYear() > this.datePicker.startDate) {
		switch(this.saveFormat.toLowerCase()){
		case "rfc":
		case "iso":
		case "":
