...
<#if parameters.list?exists>
    ...
    <!-- jbenitac: hago que compare con el nameKeyValue, en lugar del nameValue -->
    <option value="${tmpListKey?html}"<#rt/>    	
        <#if (parameters.nameKeyValue?exists && parameters.nameKeyValue == tmpListKey)>
...
<img id="ayuda_${parameters.name?html}" name="ayuda_${parameters.name?html}" src="/etir/image/iconos/16x16/field_help.png" alt="Ayuda de campo" style="vertical-align:middle;cursor:pointer" />

</#if>

<!-- jbenitac: cambio para que pinte el indicator -->
<#if parameters.indicator?if_exists != "">
	<img id="${parameters.indicator?html}" src="/etir/image/indicator.gif" alt="Cargando..." style="display:none"/>
</#if>
...
$(document).ready(function() {
	...
	$('#ayuda_${parameters.name?html}').click(function(e) {
		...
		if(
			...
			&& ($('#ayuda_${parameters.name?html}').position().top != $('#ayuda_div').position().top
			|| $('#ayuda_${parameters.name?html}').position().left + 16 != $('#ayuda_div').position().left)
			){	
